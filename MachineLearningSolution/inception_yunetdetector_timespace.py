...
class FaceDetectorYunet():
    ...
    def extract_face(self, image, face, target_size=(256, 256)):
      """Extract face from image using bounding box coordinates"""
      if isinstance(image, str):
          image = cv2.imread(str(image))
...
Extract bounded image and bounding box and feed to model
...
def create_classification_model(input_shape, num_classes):
    ...
    # We don't need a separate rescaling layer since we'll normalize the data during preprocessing
    ...
    # Compile the model
    model.compile(
        loss=keras.losses.CategoricalCrossentropy(),  # Changed to CategoricalCrossentropy since we're one-hot encoding
...
# 3. Visualize Predictions on a Subset of Validation Images
num_samples = 5
